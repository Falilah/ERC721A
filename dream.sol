...
contract Dreamer is ERC721A, Ownable {

    using Strings for uint256;

    /*//////////////////////STATE VARIABLES *///////////////////////////

    uint256 public constant WHITELIST_MAX = 5000;
    ...
    function buy(uint256 _quantity) external payable {
        ...
        uint256 mintCost;
        PublicSale memory _publicSale = publicSale;
        if (block.timestamp - startTime >= publicSale.duration) {
            mintCost = whitelistPrice * _quantity;
        } else {
            uint256 steps = (block.timestamp - startTime) /
                _publicSale.interval;
            mintCost =
                (publicSale.saleStartPrice -
                    (steps * _publicSale.decreaseRate)) *
                _quantity;
        }
    ...
    function getCurrentPrice() external view returns (uint256) {
        if (!saleActive) {
            return publicSale.saleStartPrice;
        }
        PublicSale memory _publicSale = publicSale;
        if (block.timestamp - startTime >= _publicSale.duration) {
    ...
    function activateSale() external onlyOwner {
        require(publicSale.saleStartPrice != 0, "SALE_VARIABLES_NOT_SET");
